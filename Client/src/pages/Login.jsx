            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setShowForgot(false)}>
              <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl animate-fadeIn" onClick={(e) => e.stopPropagation()}>
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Automated password resets are not enabled on this instance yet. Please contact your
                  server administrator to reset your password.
                </p>
                <button
                  onClick={() => setShowForgot(false)}
                  className="mt-5 w-full rounded-xl bg-slate-800 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-700"
                >
                  Got it
                </button>
              </div>
