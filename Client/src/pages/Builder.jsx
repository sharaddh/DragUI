...
export default function Builder() {
  ...
  const handleDragStart = ({ active }) => {
    // Capture both the raw data and fallback structural text if payload lacks a type
    const payload = active.data?.current || active.data || {};
    const fallbackType = active.id?.startsWith("tool-") ? active.id.split("tool-")[1].split("-")[0] : null;
    
    setActiveDrag({
      type: payload.type || fallbackType || "Component",
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      {/* FIXED: changed max-w-400 to max-w-[1400px] */}
      <main className="mx-auto max-w-[1400px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        >
          ...
          <DragOverlay>
            {activeDrag ? (
              <div className="pointer-events-none rounded-2xl border border-cyan-400 bg-cyan-500/95 px-5 py-3 text-sm font-semibold text-white shadow-xl shadow-cyan-500/20 capitalize">
                {activeDrag.type}
              </div>
            ) : null}
          </DragOverlay>
